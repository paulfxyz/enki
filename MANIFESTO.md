...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/references-86-a1731c?style=flat-square" alt="86 references">
...
The open-weight world has caught up. DeepSeek R1 matched OpenAI's o1-class reasoning under an MIT license [17]; Alibaba's Qwen family crossed a billion downloads [18]; Moonshot's Kimi K3 shipped fully open weights approaching frontier performance [19]. By Stanford HAI's own index, the best open models sit within a few percentage points of the best closed ones — a gap that breathes, narrowing to half a point in 2024 and widening back to roughly three in 2026, but never again an order of magnitude [20]. Open models — from Meta, Mistral, and the Chinese labs — are a genuine public good, and the value of such goods is measurable: the European Commission estimates open-source software already contributes €65–95 billion a year to EU GDP alone [48]. Yochai Benkler saw this engine early: commons-based peer production — strangers cooperating around freely shared information — can out-produce both firms and markets at exactly this kind of work [67]. There is deep precedent for what happens when knowledge infrastructure is given away rather than fenced: in 1993 CERN released the World Wide Web royalty-free, and a single signature seeded more value than any patent in history [44]. The Enki Institute will centralise open models in one tight, accessible directory, importable into the Enki device, Ollama and any compatible runtime. A model you cannot download is a promise; a model you can is property held in common.

## Article VI The local AI datacenter

The hardware for local intelligence already ships by the hundreds of millions. Roughly 59% of PCs shipping in 2026 sit in the most capable tier the analysts track — “AI-Advanced,” up from about 39% the year before [21]; by 2028, IDC expects roughly 94% of commercial PC shipments to carry an NPU [22]. Apple runs capable ~3B-parameter multimodal models on phones [23]. Ollama alone is used by more than 8.9 million developers every month [24]. A study of one million real queries found local models already answer 88.7% of everyday chat and reasoning tasks — under best-of-local routing across twenty-odd models, with lower coverage in technical domains and agentic work not yet measured; a single 2025 local model alone answers 71.3%, up from 23.2% in 2023 [25]. The trajectory is the argument. The silicon industry itself says the centre cannot hold: Qualcomm's engineering roadmap declares the future of AI hybrid, with inference migrating out of the datacenter and onto the device for cost, energy, privacy and latency alone [76].

Now extend the idea: the NPUs and GPUs already inside the devices you own, synced over your home network — your phone the natural remote, an always-on plugged-in machine the natural workhorse. A local AI datacenter, scaled from a studio flat to an entire office, carrying the everyday tier of consumer and corporate usage on hardware you already own. Small models on small devices; larger models where a device can carry them; the mesh routing each task to the machine that can run it. And let us be precise about the physics, because our case does not need exaggeration: token generation is bound by memory bandwidth, and a phone moves memory at a fraction of a datacenter GPU's pace. Heat is the second honest constraint: sustained inference throttles a phone within minutes and drains it within hours, which is why the mesh's keystone is not the device in your hand but the one plugged into the wall. What a household mesh delivers first is routing and discovery — a task that outgrows the device in your hand is dispatched to whichever of your machines can carry it, without ever leaving your network. Pooling several devices to truly share the load behind one larger model is a research ambition we intend to pursue, not a shipped feature: prima.cpp already runs 70B-parameter models across four ordinary home devices, at latencies that are honest rather than instant [80]. Peer inference across strangers has been tried and has stalled — Petals ran hundred-billion-parameter models over volunteer GPUs and remained a research curiosity, because strangers are slow and strangers must be trusted [85]. A household mesh has neither problem: every node is a device you already bought, on a network you already lock. And people have already chosen local-first at consumer scale, against every platform's preference — millions of homes run Home Assistant, stewarded by a non-profit foundation, precisely to keep their houses off someone else's cloud [86]. This mesh is the real invention — and the hardware will honour it: our objective is a device, called Enki — a box the size of a slim pocket battery, no screen, one button — that runs open models on its own silicon, meshes over WiFi with every capable machine around it, keeps its models signed-current whenever it touches a network, and presents the whole as a single localhost API/MCP endpoint any AI client can dock to. And then we will try to put it in as many human hands as possible — distributed free, or at a strict no-margin price — a portable local AI datacenter in your pocket for whenever you step outside your safe AI environments. To make this possible at scale, one piece of plumbing is still missing: an international standard under which any device shipping a GPU or NPU can have its models kept up to date and enrolled into the local mesh — and under which that mesh is recognised, out of the box, as an API/MCP gateway that any AI client can speak to, the Enki device of course included. The playbook for such a standard is proven — and so is its cautionary tale. In December 2019, Amazon, Apple, Google and Samsung SmartThings — rivals everywhere else — sat down in a single working group with the Zigbee Alliance; by October 2022 the renamed Connectivity Standards Alliance shipped Matter 1.0 with a certification programme behind it, backed by more than 550 member companies [77]. Honesty requires the sequel: adoption since has been slow and partial — platforms shipping support several versions behind the specification, certified products in the hundreds rather than the tens of thousands [33]. We cite Matter for both lessons: it can be done, and the neutral-custody clause of our standard exists because of how it went wrong. Where industry alone moves too slowly, a legislature can finish the job: the EU's common-charger directive made USB-C the law of a continent — one port for phones by the end of 2024, laptops by 2026 [78]. And half of the gateway layer already exists: MCP, the open protocol this manifesto keeps naming, went from one company's specification to adoption across ChatGPT, Gemini and Copilot inside a year, then was donated to a vendor-neutral foundation [79]. Rivals in one working group, a certification mark, a legislature where needed, a neutral home for the spec — local intelligence deserves the same instruments.

This is where the everyday tier of AI belongs: holding a conversation, learning something new, checking a fact, the news or the web, vibe coding an idea into software, generating an image, or running the automations and workflows of personal and professional life — the great majority of everyday use, growing every year as small models improve. What stays above — genuinely compute-hungry research and the frontier itself — is exactly what Article IX silos: two tiers of one architecture, not two ambitions. All the rest: local-first, device-integrated, and so cost-efficient that to a regular person it feels effectively free. And let the ecological claim be as precise as the physics: measured per token, a datacenter GPU often wrings more work from a joule than a phone ever will [25]. The case for local is different — the silicon is already manufactured and its carbon already spent, and every everyday task it absorbs is demand the next warehouse never has to be built for. The research arm of the Enki Institute will seek to make exactly that possible — committing every resource and every idea it can gather, and prioritising open-source software and secure WiFi/Bluetooth mesh networking as the substrate the whole vision runs on. The most radical infrastructure programme of this century needs no planning permission: it runs on the devices you already own.

## Article VII The standard we need

For the local AI datacenter to ship in every device rather than live in a hobbyist's weekend, one international standard has to exist. We can already name its four clauses. **Model currency:** any device shipping a GPU or NPU receives verified updates to the small models it carries — each release signed by its publisher, countersigned after validation, logged to a public transparency ledger and revocable the moment a fault is found — as routine and as trusted as security patches are today. **Mesh enrolment:** a new device joins the household mesh with a single confirmation, declares what it can carry — memory, memory bandwidth, thermal envelope — and accepts work sized to that declaration, the way Matter commissions a lightbulb in seconds [77]. **Gateway recognition:** the mesh presents itself as one endpoint speaking an open API/MCP dialect, so any AI client — Claude, Perplexity, LibreChat, ChatGPT, or whichever assistant you prefer — can plug into it without a driver, a subscription or anyone's permission [79]. **Neutral custody:** the specification lives in a vendor-neutral body where rivals hold each other honest, with a certification mark consumers can trust and, where industry stalls, a legislature willing to finish the job as Europe did with the common charger [78]. This clause is written in Matter's ink: slow platform adoption and partial implementations are what happen when custody is soft [33].

And we can already describe how it runs, because every part is proven somewhere. A model ships like a container image: a signed, content-addressed package pulled from a public registry — the pattern Ollama already uses to distribute models today [83] — published in several quantisations and in the formats each accelerator demands [84] — one index serving many kinds of silicon, exactly as container registries already serve ARM and x86 from a single name [83] — so each device pulls the variant its declared capability can carry: the phone takes the 4-bit 3-billion compiled for its NPU, the desktop GPU takes the 27-billion. A device announces itself on the home network with the same zero-configuration discovery that lets a laptop find a printer or a Chromecast — mDNS/DNS-SD, standardised since 2013 and running in billions of devices [82]. Discovery is a hint, never an authority: an announcement tells the mesh where a device might be, but every connection is mutually authenticated against keys exchanged at enrolment, and nothing shouted on a network can override them. The gateway — any capable machine in the house; the Enki device is one implementation — keeps the map of who holds what and routes each request, whole, to the one device that can serve it — session-aware, so a conversation stays pinned to the machine already holding its context — and the reply never leaves your network. Distribution pays for itself the way software updates already do: a model pulled once into a household is served from inside it to every other device, mirrors and ISP caches carry the rest, and the more homes join, the cheaper the commons becomes — the exact inverse of datacenter economics. And when a model outgrows every single device, its layers can be split across several — pipeline parallelism over ordinary WiFi, which prima.cpp already demonstrates with 70B models on four home machines [80]; the capability declarations of mesh enrolment are exactly what such a scheduler needs to place each slice. Registry, discovery and routing are solved problems in adjacent domains, waiting to be pointed at models. Two pieces are genuinely unbuilt — a capability record precise enough to schedule against, and the matching layer that serves one signed model to many kinds of silicon — and aggregation remains live research, not a shipped guarantee. The remainder is the least glamorous work in technology: writing the specification, testing conformance, and getting rivals to sign the same page. That is what standards bodies are for.
...
And where philosophy points, evidence follows. Elinor Ostrom won the Nobel prize for documenting, across centuries and continents, that commons governed by their own members — clear boundaries, real monitoring, rules set by the people who live under them — reliably outlast both state control and privatisation [43]. That finding is the constitutional physics of the 300 — a commons governed by its members, where every seat is held on contribution and none is owned: in the annual audit that swaps thirty out for thirty in, even the founder, Paul Fleury, can be voted out like anyone else. A public good has no owner, and no ego worth protecting. The internet was engineered on the end-to-end argument — keep the network simple, put the intelligence at the edges [45] — and the web became the largest shared artefact our species has built because CERN signed it over to humanity, royalty-free, in 1993 [44]. Local-first computing restates the same principle for the age of models [46]. Everything this letter proposes is an application of that lineage — nothing here is invented; it is inherited.
...
85. [85] Alexander Borzunov et al., “Petals: Collaborative Inference and Fine-tuning of Large Models,” arXiv:2209.01188, 2023 — arxiv.org — https://arxiv.org/abs/2209.01188

86. [86] Open Home Foundation — the non-profit steward of Home Assistant, founded April 2024 — openhomefoundation.org — https://www.openhomefoundation.org/
